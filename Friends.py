        ...
        if(r.status_code==200):
            ...
            for td in table.findAll('td',{'class':'Icon'}):
                ...
                if("/" not in Friend): #To not get invalid values
                    mainBuddies = mainBuddies + Friend + ","
    return mainBuddies
...
#Taking the amount of this contact's friends
def getBuddies(User, mainFriends):
    try:
        pages = getPages(User)
        for i in range(1,pages):
            r = requests.get("http://www.flickr.com/people/" + User + "/contacts/?filter=&page=" + str(i))
            if(r.status_code==200):
                ...
                for td in table.findAll('td',{'class':'Icon'}):
                    ...
                    Friend = href[8:-1]
                    if("/" not in Friend): #To not get invalid values
                        if(Friend in mainFriends):
                            writeFile(User,Friend)
                    if(str(User) == "j0annie"):
                        getBuddies(str(Friend), mainFriends)
            print i 
    except Exception:
        print "exception"
    else: 
...
mainContact = "j0annie"
mainBuddies = getUsers(mainContact)
getBuddies(mainContact, mainBuddies)
